...
variants:
    ...
    - name: Langue et format
      description: >
          `lang` pilote à la fois les noms de mois/jours et les libellés des boutons «
          Aujourd'hui » / « Fermer » (mécanisme i18n) — une seule source de vérité, jamais de
          composant à moitié traduit. `format` définit, séparément, le motif de saisie/affichage
          attendu dans l'input (indépendant de la langue). Le texte du hint par défaut ("Format
          attendu :", "ex.") ne suit pas `lang` — à traduire via le slot `hint`.
      html: |
          <ar-datepicker
            label="Date of birth"
            format="MM/dd/yyyy"
