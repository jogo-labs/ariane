...
variants:
    ...
    - name: Langue et format
      description: >
          `lang` pilote à la fois les noms de mois/jours, les libellés des boutons «
          Aujourd'hui » / « Fermer » et le hint par défaut ("Format attendu :", "Dates
          disponibles :") — une seule source de vérité, jamais de composant à moitié traduit.
          `format` définit, séparément, le motif de saisie/affichage attendu dans l'input
...
[Internationalisation](/getting-started/i18n)) — le même attribut que celui qui pilote les noms de
mois/jours du calendrier (pas de prop `locale` séparée : une seule langue pour tout le composant).
Sans `lang` explicite, la langue vient de `<html lang>` puis de `navigator.language` ; le français
sert de repli quand la langue résolue n'a pas de traduction enregistrée.
