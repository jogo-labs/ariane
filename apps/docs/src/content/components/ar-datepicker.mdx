...
variants:
    ...
    - name: Locale et format
      description: >
          ...
          français quelle que soit `locale` — à traduire via le slot `hint`. Les libellés des
          boutons « Aujourd'hui » / « Fermer » suivent `lang` (mécanisme i18n), indépendamment de
          `locale`.
      html: |
          <ar-datepicker
            label="Date of birth"
...
[Internationalisation](/getting-started/i18n)) — pas `locale`, qui ne pilote que le formatage des
dates. Sans `lang` explicite, la langue vient de `<html lang>` puis de `navigator.language` ; le
français sert de repli quand la langue résolue n'a pas de traduction enregistrée.
