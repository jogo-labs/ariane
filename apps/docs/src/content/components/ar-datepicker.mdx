...
`--ar-datepicker-input-max-width` vaut par défaut une valeur calculée à partir de la longueur de `format`, indépendante de la largeur du hint (qui peut rester plus large). Si vous la surchargez, gardez-la cohérente avec le contenu attendu dans le champ, notamment évitez d'agrandir le champ de manière exagérée.
