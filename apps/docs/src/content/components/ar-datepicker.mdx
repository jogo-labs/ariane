    ...
    - name: Plage de dates
      ...
      html: |
          <ar-datepicker
            ...
            lang="fr-FR"
          ></ar-datepicker>
    - name: Langue et format
      description: >
          L'attribut `lang` pilote la totalité des textes et formats — y compris les textes par
          défaut des slots — à l'exception du label de l'input.
      html: |
          <ar-datepicker
            label="Date of birth"
            format="MM/dd/yyyy"
...
- **Hint** : utiliser le slot `hint` pour communiquer le format attendu — ne pas utiliser `placeholder` à cet effet (il disparaît à la saisie). Un contenu personnalisé remplace entièrement le hint par défaut, y compris l'exemple de date et la mention de la plage `min`/`max` : à répéter manuellement si nécessaire. Le hint par défaut suit `lang` comme le reste du composant (mécanisme i18n) — le slot reste utile pour un contenu personnalisé ou une langue non traduite par le package.
...
- **Focus des jours** : l'indicateur de focus visible sur la grille (<WcagRef criterion="2.4.7" summary="Focus Visible : le composant recevant le focus clavier doit être visuellement identifiable." />) est porté par `--ar-datepicker-day-focus-ring-color`/`-width`/`-offset`, avec leurs propres replis. `--ar-datepicker-day-focus-border-color` est un token distinct, purement cosmétique — le redéfinir ou le supprimer n'affecte pas la garantie WCAG 2.4.7, portée uniquement par l'anneau de focus.
- **Largeur de l'input** : `--ar-datepicker-input-max-width` vaut par défaut une valeur calculée à partir de la longueur de `format`, indépendante de la largeur du hint (qui peut rester plus large). Si vous la surchargez, gardez-la cohérente avec le contenu attendu dans le champ — trop étroite, elle tronque visuellement la date saisie.
...
En complément de `min`/`max` (voir la variante « Plage de dates »), le callback `isDateDisabled`
couvre des règles arbitraires (week-ends, jours fériés…) :

```js
el.isDateDisabled = (d) => d.getDay() === 0 || d.getDay() === 6; // week-ends
```

Les jours désactivés reçoivent `aria-disabled="true"` (jamais l'attribut natif `disabled`, pour
rester dans la grille de navigation). Contrairement à `min`/`max`, `isDateDisabled` n'est pas décrit
dans le hint par défaut : ses règles étant arbitraires, il n'existe pas de formulation générique — à
documenter via le slot `hint` si nécessaire.
...
Ces deux textes suivent le mécanisme i18n du package (attribut `lang`, cf.
[Traductions](/getting-started/traductions)).

```html
<ar-datepicker label="Date of birth" lang="en"></ar-datepicker>
```

Pour un contenu plus riche (icône + texte), les slots `today-label` / `close-label` remplacent le contenu du bouton. Le texte traduit reste alors la source du `aria-label` posé sur le `<button>`, ce qui garantit un nom accessible même si le slot ne contient qu'une icône :

```html
<ar-datepicker lang="en">
