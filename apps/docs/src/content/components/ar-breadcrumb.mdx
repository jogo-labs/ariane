...
title: Breadcrumb
description: >-
    Affiche un fil de liens qui montre à l'utilisateur sa position dans la hiérarchie du site,
    avec un affichage adapté au mobile et au desktop.
variants:
    - name: default
      label: Par défaut
...
`::part(list)` et `::part(home)` localement pour garantir la lisibilité (le bouton "Retour"
mobile tire sa couleur de texte de `::part(home)`, pas de `::part(list)`) :
