...
variants:
    - name: default
      ...
      html: |
          <button class="ar-btn ar-btn-primary" data-ar-dialog-open="demo-modal">Ouvrir la modale</button>
    ...
    - name: drawer
      ...
      html: |
          <button class="ar-btn ar-btn-primary" data-ar-dialog-open="demo-drawer">Ouvrir le drawer</button>
    ...
    - name: drawer-left
      ...
      html: |
          <button class="ar-btn ar-btn-primary" data-ar-dialog-open="demo-drawer-left">Ouvrir le drawer gauche</button>
    ...
    - name: close-on-backdrop
      ...
      html: |
          <button class="ar-btn ar-btn-primary" data-ar-dialog-open="demo-backdrop">Ouvrir (close-on-backdrop)</button>
    ...
    - name: with-footer
      ...
      html: |
          <button class="ar-btn ar-btn-primary" data-ar-dialog-open="demo-footer">Ouvrir avec footer</button>
          <ar-dialog id="demo-footer" label="Confirmer la suppression">
              <p>Cette action supprimera définitivement l'élément sélectionné.</p>
              <button class="ar-btn ar-btn-secondary" slot="footer" data-ar-dismiss>Annuler</button>
              <button class="ar-btn ar-btn-danger" slot="footer" data-ar-accept>Supprimer</button>
          </ar-dialog>
    - name: header-actions
      ...
      html: |
          <button class="ar-btn ar-btn-primary" data-ar-dialog-open="demo-header-actions">Ouvrir</button>
          <ar-dialog id="demo-header-actions" label="Rapport mensuel">
              <button class="ar-btn ar-btn-secondary" slot="header-actions" aria-label="Passer en plein écran" style="font-size: 1.5rem">⤢</button>
    ...
    - name: header-actions-advanced
      ...
      html: |
          <button class="ar-btn ar-btn-primary" data-ar-dialog-open="demo-header-advanced">Ouvrir le drawer</button>
          <ar-dialog id="demo-header-advanced" class="demo-header-advanced" label="Titre accessible du volet" mode="drawer">
              <div slot="header-actions" class="demo-header-advanced-actions">
                  <div class="actions">
                      <button class="ar-btn ar-btn-primary">Action 1</button>
                      <button class="ar-btn ar-btn-secondary">Action 2</button>
                  </div>
    ...
    - name: without-header
      ...
      html: |
          <button class="ar-btn ar-btn-primary" data-ar-dialog-open="demo-without-header">Ouvrir</button>
    ...
    - name: autofocus
      ...
      html: |
          <button class="ar-btn ar-btn-primary" data-ar-dialog-open="demo-autofocus">Ouvrir le drawer</button>
    ...
    - name: stacked
      ...
      html: |
          <button class="ar-btn ar-btn-primary" data-ar-dialog-open="demo-stacked-1">Ouvrir le dialog</button>
          <ar-dialog id="demo-stacked-1" label="Dialog principal">
              <p>Ce dialog peut en ouvrir un second par-dessus.</p>
              <button class="ar-btn ar-btn-secondary" data-ar-dialog-open="demo-stacked-2">Ouvrir un second dialog</button>
          </ar-dialog>
    ...
    - name: persistent
      ...
      html: |
          <button class="ar-btn ar-btn-primary" data-ar-dialog-open="demo-persistent">Ouvrir le dialog persistant</button>
          <ar-dialog id="demo-persistent" label="Action requise" prevented-message="Fermeture bloquée — utilisez le bouton Confirmer.">
              ...
              <button class="ar-btn ar-btn-primary" slot="footer" data-ar-accept>Confirmer</button>
          </ar-dialog>
          <script>
              (function () {
...
Le conteneur du slot `header-actions` expose son propre `::part(header-actions)`, indépendant du layout titre/fermeture — utile pour définir un espacement entre plusieurs actions sans affecter le reste du header :
...
propre layout interne (actions à gauche, pagination à droite, titre sr-only…). Voir la variante
« Header avancé » pour une démonstration complète.

Pour retirer entièrement le header, voir la variante « Sans header ».

### Cycle d'événements

Tous les événements portent `detail: { id }` (l'id du dialog). `data-ar-dismiss` et `data-ar-accept` sont deux conventions distinctes de `ar-dialog-hide` : elles permettent de distinguer une annulation ("Annuler") d'une confirmation ("Confirmer") sur le même geste de fermeture. La liste complète des événements, avec leur annulabilité, est dans la [Référence API](#reference-api).
...
Le verrouillage du scroll de la page est partagé entre tous les dialogs ouverts : il n'est relâché que lorsque le dernier dialog de la pile se ferme. Voir la variante « Dialogs empilés » pour le comportement de `Escape` et la coordination du focus.
