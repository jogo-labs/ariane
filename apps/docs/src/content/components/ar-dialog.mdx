...
« Header avancé » ci-dessus pour une démonstration complète.

Pour retirer entièrement le header, voir la variante « Sans header » ci-dessus.
...
Le verrouillage du scroll de la page est partagé entre tous les dialogs ouverts : il n'est relâché que lorsque le dernier dialog de la pile se ferme. Voir la variante « Dialogs empilés » ci-dessus pour le comportement de `Escape` et la coordination du focus.
