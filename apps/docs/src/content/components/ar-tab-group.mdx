...
- Les techniques de signalement d'overflow (`mask-image`, boutons de scroll via `scrollNav()`) sont démontrées dans les variantes _Scroll hints_ et _Boutons de scroll_ ci-dessus.
...
Par défaut (`manual-activation` absent), les flèches déplacent le focus **et** activent immédiatement l'onglet ciblé — `ar-tab-group-change` se déclenche à chaque flèche. Voir la variante _Activation manuelle_ ci-dessus pour le comportement avec `manual-activation`.
