...
outline-offset: 2px;`.
- `stepper.styles.ts` : `.item-header:focus-visible` (s'applique aussi bien au `<div>` qu'à
  l'intérieur d'un `<a>` existant, sans changement pour ce dernier qui a déjà son propre style de
  focus sur `[part~='step-link']:focus`) — `outline: 2px solid var(--ar-stepper-link-focus-outline-color);
outline-offset: 4px;` (mêmes valeurs que la règle `:focus` déjà posée sur `[part~='step-link']`,
  pour rester visuellement cohérent entre les deux états).
